...
import type { OperationalIncidentRpc } from "../_shared/operational-incidents.ts";
import { createReportRuntimeIncidentHandler } from "./handler.ts";
const supabaseUrl = Deno.env.get("SUPABASE_URL");
const serviceRoleKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
const rpc: OperationalIncidentRpc = async (name, parameters) => {
...
const handler = createReportRuntimeIncidentHandler({
  environment: {
    ...
    runtimeIncidentAllowedOrigins:
      Deno.env.get("RUNTIME_INCIDENT_ALLOWED_ORIGINS") ??
