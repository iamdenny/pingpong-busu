create or replace function public.is_individual_division_record(
...
parallel safe
as $$
  select coalesce(p_event_type, 'unknown'::public.event_type) not in (
      'doubles'::public.event_type,
      'team'::public.event_type
    )
    and coalesce(p_event_name, '') !~ '(복식|단체|혼합|혼성)';
$$;
...
comment on view public.public_player_search is
  'Public player search summaries using individual-event division observations, excluding future tournaments, and preferring standard integrated divisions on same-day records.';
