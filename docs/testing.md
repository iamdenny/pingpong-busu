...
- 복수 후보에서 `전체 N건 한 사람으로`가 모든 후보를 첫 별칭에 배정하고 배정 후 비활성화되는가. `전체 해제`가 모두 `모름`으로 되돌리는가. 전체가 한 별칭에 묶인 상태에서 확인 문구가 전체 병합 안내로 바뀌는가. mobile에서 두 버튼이 안내 문구 아래로 쌓이며 44px 이상 tap 영역을 유지하는가
