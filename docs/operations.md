...
36. `202608210001_player_seo_record_snapshot.sql`: 선수 기록 스냅샷을 materialized view로 미리 계산하고 SEO manifest가 색인된 스냅샷을 join하도록 변경. 야간 pg_cron 갱신과 service-role 갱신 함수 포함
...
Pages build는 같은 `VITE_SUPABASE_URL`과 `VITE_SUPABASE_PUBLISHABLE_KEY`로 경량 공개 `public_player_seo_manifest` view만 읽어 선수 SEO 스냅샷을 만든다. main CI의 production read 게이트는 이번 release의 migration이 적용되기 **전**의 운영 DB에 새 번들을 연결하므로, manifest에 새 컬럼을 추가할 때는 구 DB에서도 build가 성공해야 한다. generator는 기록 컬럼이 없다는 PostgREST 400(SQLSTATE 42703)을 만나면 식별 컬럼만으로 한 번 더 조회해 요약만 있는 문서를 만들고, 그 외 오류는 그대로 build를 실패시킨다. manifest를 넓힐 때는 배포가 view를 전부 열거한다는 점을 먼저 확인한다. 선수당 lateral 서브쿼리를 더한 `202608200001`은 운영에서 `limit 1`조차 statement timeout(57014)을 넘겨 `202608200002`로 되돌렸다. 비슷한 변경은 `pnpm db:replay`로 로컬 PostgreSQL에 전체 migration을 먼저 적용해 검증하고, 열거 비용이 늘어나는 변경은 view가 아니라 미리 계산한 스냅샷에 둔다. `pnpm db:replay`는 Docker가 필요하며 pg_cron 등 관리형 전용 확장은 대체한다. `--seed`를 붙이면 seed까지 적용한다. workflow의 `SEO_MANIFEST_REQUIRED=true`는 운영 전용 fail-closed 스위치이며 별도 secret이 아니다. 설정 누락, 공개 API 오류, 행 검증 실패, 빈 manifest면 build와 배포를 중단한다. service role/secret key 또는 private table 접근을 이 단계에 추가하지 않는다.
