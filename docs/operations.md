...
잘못된 최신 편집은 검색 화면의 `참여 편집 이력 → 되돌리기`에서 누구나 10자 이상의 근거를 남겨 전체 원복할 수 있습니다. 기존 편집자의 코드나 동일 브라우저일 필요는 없습니다. `revert-identity-edit`은 참여 편집으로 생성된 작업만 허용하고 내부 `revert_identity_partition_internal`을 호출해 해당 편집의 그룹 병합과 별칭을 모두 되돌립니다. 후속 편집이나 현재 연결 충돌이 있으면 덮어쓰지 않고 충돌을 반환하며 최신 편집부터 역순으로 되돌립니다.
