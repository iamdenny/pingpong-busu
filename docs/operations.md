...
아이핑 queue 운영 상태는 `Scheduled iPing refresh worker` Actions 실행과 private `refresh_jobs` 집계로 확인합니다. `queued`가 계속 증가하면 workflow secret/variable과 Edge 배포 상태를 먼저 확인합니다. `running`의 lease가 4분을 넘으면 다음 claim이 자동 회수합니다. `source_backlog_stopped`나 화면의 `보호 대기`가 나타나면 먼저 인증·구조·접근 차단 원인을 해결하고 `sources.enabled`, `CRAWL_LIVE`, `CRAWLER_SOURCE_IPING_ENABLED`, 계정 Secret을 확인합니다. 계정 Secret을 바꿨다면 Supabase 배포 성공만으로 기존 DB 회로가 해제되지 않으므로, main의 `Scheduled iPing refresh worker`를 수동 실행해 mode를 `recover-iping`으로 한 번 선택합니다. main이 아닌 ref의 수동 실행은 검증 단계에서 실패합니다. 이 모드는 정상 예약 실행에서는 사용되지 않으며 service-role RPC가 최근 24시간의 결정적 실패 작업 한 건만 재예약한 뒤 즉시 기존 worker로 검증합니다. 실제 작업이 성공한 경우에만 workflow가 통과하고 다음 검색에서 갱신 결과를 확인할 수 있습니다. 다른 worker가 실행 중인 `busy`, 재예약 대상이 없는 `reset_only`, 일시 오류로 재시도만 예약된 경우는 검증 미완료로 workflow가 실패합니다. 다시 인증에 실패하면 workflow가 실패하고 6시간 회로가 재개방되므로 반복 실행하지 말고 계정 자체를 다시 확인합니다. `busy`는 다른 worker의 lease를 건드리지 않으며, 대상 실패가 없으면 회로만 초기화해 다음 사용자 검색이 새 작업을 만들게 합니다. 긴급 중지는 두 스위치를 false로 내려 새 enqueue와 drain을 모두 막고 저장 기록은 삭제하지 않습니다. token 회전 시에는 새 repo secret으로 배포 workflow를 실행해 Edge secret을 먼저 동기화한 뒤 예약 workflow를 재개합니다.
