...
37. `202608210003_seo_snapshot_refresh_observability.sql`: 선수 기록 스냅샷 갱신을 staging 단계로 재작성해 비용을 낮추고, 취소된 실행까지 남기는 공개 실행 기록 테이블 추가
38. `202608210002_trending_player_views.sql`: 홈 조회 순위를 위한 시간 단위 집계 `player_view_counts`·`player_view_origins`, 상위 10명만 노출하는 `public_trending_players` view, service-role 전용 기록·정리 RPC와 매시 정리 pg_cron
...
Pages build는 같은 `VITE_SUPABASE_URL`과 `VITE_SUPABASE_PUBLISHABLE_KEY`로 경량 공개 `public_player_seo_manifest` view만 읽어 선수 SEO 스냅샷을 만든다. main CI의 production read 게이트는 이번 release의 migration이 적용되기 **전**의 운영 DB에 새 번들을 연결하므로, manifest에 새 컬럼을 추가할 때는 구 DB에서도 build가 성공해야 한다. generator는 기록 컬럼이 없다는 PostgREST 400(SQLSTATE 42703)을 만나면 식별 컬럼만으로 한 번 더 조회해 요약만 있는 문서를 만들고, 그 외 오류는 그대로 build를 실패시킨다. manifest를 넓힐 때는 배포가 view를 전부 열거한다는 점을 먼저 확인한다. 선수당 lateral 서브쿼리를 더한 `202608200001`은 운영에서 `limit 1`조차 statement timeout(57014)을 넘겨 `202608200002`로 되돌렸다. 비슷한 변경은 `pnpm db:replay`로 로컬 PostgreSQL에 전체 migration을 먼저 적용해 검증하고, 열거 비용이 늘어나는 변경은 view가 아니라 미리 계산한 스냅샷에 둔다. `pnpm db:replay`는 Docker가 필요하며 pg_cron 등 관리형 전용 확장은 대체한다. `--seed`를 붙이면 seed까지 적용한다. 배포 연결에도 statement timeout이 있으므로 migration은 큰 집계를 실행하지 않는다. 선수 기록 스냅샷은 `refresh_player_seo_record_snapshot()`이 자체 timeout을 올려 staging에 계산한 뒤 한 번의 insert로 교체하며, 매일 02:41에 pg_cron이 실행한다. 스냅샷이 비어 있으면 정적 선수 문서는 기록 없이 요약만 싣는다. 상태는 publishable key로 `player_seo_record_snapshot`의 행 수와 `refreshed_at`, 그리고 `player_seo_snapshot_runs`의 최근 행을 조회해 확인한다. `player_seo_snapshot_runs`가 비어 있으면 pg_cron이 실행되지 않은 것이고, `failed_reason`이 있으면 실행은 됐으나 끝내지 못한 것이다. 두 원인은 대응이 다르므로 먼저 이 표를 본다. workflow의 `SEO_MANIFEST_REQUIRED=true`는 운영 전용 fail-closed 스위치이며 별도 secret이 아니다. 설정 누락, 공개 API 오류, 행 검증 실패, 빈 manifest면 build와 배포를 중단한다. service role/secret key 또는 private table 접근을 이 단계에 추가하지 않는다.
...
아이핑 queue 운영 상태는 `Scheduled iPing refresh worker` Actions 실행과 private `refresh_jobs` 집계로 확인합니다. `queued`가 계속 증가하면 먼저 Repository Settings → Actions → Runners에서 `iping` 러너가 `Idle` 또는 `Active`인지 확인하고, 이어서 production의 계정 Secret, repository의 worker token·project variable, Edge 배포 상태와 컨테이너의 `/usr/bin/chromium` 실행 권한을 봅니다. 러너가 `Offline`이면 Docker Desktop과 `busu-iping-runner` 컨테이너를 순서대로 다시 시작한 뒤 예약 실행을 기다리거나 수동 drain을 한 번 실행합니다. 이미 등록된 컨테이너는 재시작할 때 registration token을 다시 요구하지 않습니다. 재생성할 때만 GitHub의 단기 registration token을 표준 입력으로 전달하며 환경 변수·image·저장소에 남기지 않습니다. runner 컨테이너에는 host directory나 Docker socket을 mount하지 않고 다른 저장소나 pull request workflow에 `iping` 라벨을 사용하지 않습니다. production environment의 deployment branch도 `main`으로 제한합니다. worker 실패 로그는 허용 목록의 `오류 코드:단계`만 출력하고 검색어·자격증명·URL·HTML·원문 오류는 출력하지 않습니다. 브라우저 로그인 응답은 비회원 문서로 먼저 도착한 뒤 client redirect로 회원 홈이 되므로, worker는 회원 또는 사람 확인 화면이 확정될 때까지 최대 12초 동안 0.5초 간격으로 다시 읽고 그 뒤에도 비회원이면 인증 실패로 기록합니다. 검색 화면도 결과 표가 확정될 때까지 최대 5초 동안 같은 방식으로 읽습니다. `running`의 lease가 4분을 넘으면 다음 claim이 자동 회수합니다. `source_backlog_stopped`나 화면의 `보호 대기`가 나타나면 먼저 인증·구조·접근 차단 원인을 해결하고 `sources.enabled`, `CRAWL_LIVE`, `CRAWLER_SOURCE_IPING_ENABLED`, 계정 Secret을 확인합니다. 예약 drain은 5분마다 한 작업을 처리하고, 같은 workflow의 매시 7분 예약이 `recover-iping`을 한 번 실행하므로 결정적 실패 회로가 배포 없이도 한 시간 안에 다시 시도됩니다. GitHub 예약은 best-effort라 실제 실행 간격이 20~80분까지 벌어지는 것을 2026-08-17에 확인했으므로, 실제 5분 주기는 러너 호스트의 cron이 `ops/iping-runner/dispatch-drain.sh`로 `workflow_dispatch`를 보내 유지하고 workflow의 예약 cron은 호스트가 꺼졌을 때의 backstop으로 둡니다. 이 스크립트는 실행 중이거나 대기 중인 실행이 있으면 dispatch하지 않으며 토큰은 호스트 사용자 전용 파일에서만 읽습니다. 대기열이 밀리면 Actions 목록에서 `workflow_dispatch` 실행이 5분 간격으로 보이는지 먼저 확인합니다. main의 `Deploy Supabase backend`가 성공하면 `Scheduled iPing refresh worker`가 같은 commit SHA로 자동 기동하고, service-role RPC가 최근 24시간의 결정적 실패 작업 한 건만 재예약한 뒤 system Chromium으로 즉시 검증합니다. 이미 작업 중인 `busy`와 복구 대상이 없는 `reset_only`는 외부 조회 없는 정상 no-op입니다. 계정 Secret만 바꿔 Edge 재배포가 필요하지 않은 경우에는 main의 같은 workflow를 수동 실행해 mode를 `recover-iping`으로 한 번 선택합니다. 실제 작업의 재시도 예약 또는 재인증 실패는 workflow를 실패시키므로 반복 실행하지 말고 원인을 확인합니다. 긴급 중지는 환경 변수와 DB source 스위치를 false로 내려 새 enqueue와 claim을 모두 막고 저장 기록은 삭제하지 않습니다. token 회전 시에는 새 repo secret으로 Supabase 배포 workflow를 실행해 Edge secret을 먼저 동기화하면 후속 자동 복구가 실행됩니다.
