...
정적 `index.html`은 홈 메타데이터를 제공한다. build의 SEO generator는 검색용 집계와 분리된 경량 공개 `public_player_seo_manifest` view를 publishable key로 페이지 단위 조회하고, 활성 공개 출처가 있는 유효 선수마다 `players/{uuid}/index.html`을 만든다. 각 문서는 초기 HTML부터 선수별 title, description, canonical, Open Graph와 Twitter large image 메타데이터를 가지며 React가 부팅된 뒤에도 공유 metadata builder로 같은 정책을 유지한다. 검색 직접 접근 문서 `search/index.html`과 client metadata는 `noindex,follow`이고 query를 canonical에서 제외한다. `sitemap.xml`에는 홈과 정적 색인, `/guide/`, 생성된 선수가 들어가고 URL마다 마지막 기록 확인일을 `lastmod`로 붙이며 `robots.txt`가 이를 가리킨다. manifest view는 배포 때 전부 열거되므로 선수당 계산을 view 안에 두지 않는다. 최근 관측 부수·부수 체계, 최근 입상 최대 5건, 공개 출처 이름, 마지막 확인 시각은 야간 pg_cron이 갱신하는 테이블 `player_seo_record_snapshot`에 미리 계산해 두고, 공개 view는 공개 id로 색인된 그 스냅샷을 left join만 한다. migration은 빈 테이블만 만들고 계산은 하지 않는다. 갱신은 결과를 staging 임시 테이블에 단계별로 만들어 plpgsql `effective_division_system` 호출을 결과당 한 번으로 줄이고, 모든 실행 결과를 공개 `player_seo_snapshot_runs`에 남긴다. 배포가 스냅샷 계산 시간에 좌우되면 안 되기 때문이며, 빈 테이블은 바로 조회되므로 첫 갱신 전에는 기록 없는 요약 문서가 나간다. 스냅샷은 RLS 대상이 아니므로 anon 정책과 같은 조건을 쿼리에 명시한다. generator는 기록 컬럼이 없거나(migration 이전) 넓은 조회가 취소되면 식별 컬럼만으로 물러나 요약 문서를 만든다. generator는 같은 스냅샷으로 부수 체계를 설명하는 정적 `/guide/` 문서와 평문 `llms.txt`를 만들고, `robots.txt`에는 검색·생성형 크롤러를 이름으로 허용한다.
