...
| `apps/web/src/lib`                                | `repository.ts`, `SupabasePlayerRepository.ts`, `runtime-incident-repository.ts`, `appVersion.ts` | 실행 모드, 데이터 접근, 오류 보고, build 버전 경계          | runtime/unit tests, public view schema             |
