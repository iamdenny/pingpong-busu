...
| `scripts`                                         | crawler, edge sync, DB size, `release-version.ts`, `generate-seo-pages.ts`, `seo-directory.ts`, `seo-html.ts`                             | 로컬/운영 도구, 릴리즈 버전, 공개 선수 SEO 스냅샷과 초성별 정적 색인 생성          | commands/operations docs, root unit tests             |
