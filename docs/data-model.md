...
`results.division_value`는 `4부`, `A부`, `T5` 같은 관측값이고 `results.division_system`은 `open`, `integrated`, `women`, `regional`, `division`, `unknown` 중 하나입니다. 같은 숫자라도 서로 다른 체계는 합산하지 않습니다. 판정 우선순위는 대회별 수동 override → T1~T7/디비전 → 명시된 지역·통합·오픈 체계 → 지역과 대회일을 함께 사용한 전환일 규칙 → 여자·여성 → 일반 숫자·문자 부수입니다. 지역별 전환일 전에는 일반 부수와 `지역0~4부` 같은 종목 내부 지역 범위를 `regional`, 당일부터는 `integrated`로 표시하고 여자 종목은 당일부터 `women`으로 표시합니다. `women`은 사용자 화면에서 `통합부수 여자6부`처럼 표현합니다. 공개 view는 출처 category·scale 등 view에 없는 근거로 파서가 확정한 `open`·`regional` 관측 체계를 우선 보존하고, 대회명·종목명에서 직접 확인한 지역과 날짜로 전환 규칙만 보완합니다. 선수 단위의 출처 지역이나 공유 대회 지역은 개별 기록의 지역 provenance가 아니므로 부수 판정에 사용하지 않습니다. 소스 관측값 자체를 변경하지 않아 가짜 revision을 만들지 않습니다. 현재 제18회까지의 분당구청장기는 대회별 override로 `regional`입니다. 지역 또는 대회일이 없으면 일반 숫자 부수는 `integrated`가 기본이고, 부수 값 자체가 없거나 해석할 수 없는 값은 `unknown`으로 보존합니다. 구체적인 기준일과 근거는 [부수 체계 전환 기준](division-transition-rules.md)에서 관리합니다.
