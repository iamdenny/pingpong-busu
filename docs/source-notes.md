...
| iping       | <https://www.iping.club/?pg=Search>                                   |      http | 인증형 / opt-in       | iping-3       | CP949 로그인 세션과 현재 로그인·로그아웃 화면 식별로 전국오픈·시군구 입상과 출전 이력 조회                                                                                |
...
2026-08-13 공개 구조 재점검에서 오케이핑퐁은 결과가 없을 때도 8열 표 안에 `colspan=8` 안내 행을 반환하는 것을 확인했습니다. 이 행은 구조 변경이 아니라 정상 0건으로 처리합니다. 에어핑퐁은 합성 검색에서 정상 HTML을 반환했지만 Supabase Edge와 출처 사이의 응답이 장시간 지연되는 사례가 있어 Edge에서는 단일 시도를 유지합니다. 2026-08-14에는 정상 응답이 5초를 넘는 사례를 수용하도록 Edge 제한을 10초로 늘렸고 화면의 최소 5초 간격·최대 2회 재시도는 유지했습니다. 별도 진단용 workspace live CLI adapter는 16초 timeout과 일시 오류 1회 재시도를 유지합니다. 아이핑은 로그인 성공 판정을 과거 `mb_logout.php` 문자열 하나에 고정하지 않고 현재 로그인 폼·로그아웃 링크·사람 확인 화면을 구분하도록 변경했습니다. 아이핑의 출처 전체 60초 잠금은 `출처 + 정규화 검색어`별 5~60초 제한으로 대체했고 계정 보호를 위해 분당 실제 요청 2회의 별도 전체 예산을 적용합니다. 에어핑퐁에는 이름을 바꾼 요청에도 적용되는 분당 6회 출처 예산을 둡니다. 2026-08-14부터 한 로그인 세션의 세 검색을 순차 실행하고 인증·구조 오류가 연속 2회 발생하면 10분 회로를 열며 성공 또는 회로 만료 시 초기화합니다. 진단에는 허용된 단계·오류 코드·소요 시간·시각만 보관하고 매일 14일 초과분을 삭제하며 검색어·응답·쿠키·자격증명은 저장하지 않습니다.
