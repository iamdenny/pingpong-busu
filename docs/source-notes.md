...
| iping       | <https://www.iping.club/?pg=Search>                                   |      http | 인증형 / opt-in       | iping-5       | Cookie/form token 분리 로그인과 예약 큐로 전국오픈·시군구 입상과 출전 이력 조회                                                                                           |
...
2026-08-12 확인 결과 아이핑 메인과 대회 목록은 비회원에게 공개되지만 `pg=Search&SchVal=...` 선수 검색은 로그인 화면으로 전환됩니다. 로그인 후 검색 화면은 전국오픈 입상(`Ctype=A`), 시군구 입상(`Ctype=B`), 출전 이력(`B=Y`)을 분리하고 선수명·관측 부수·소속·대회명·대회일·종목·순위를 표로 제공합니다. 사이트는 CP949/EUC-KR 쿼리와 응답을 사용합니다. 2026-08-16 운영 점검에서 같은 계정과 form body를 사용한 Edge HTTP 요청과 GitHub-hosted Chrome은 인증에 실패하고 로컬 네트워크의 실제 Chromium은 성공했습니다. production 수집은 main 전용 GitHub Actions를 유지하되 `self-hosted`, `Linux`, `ARM64`, `iping` 라벨을 모두 가진 저장소 전용 격리 Docker 러너의 system Chromium에서만 실행합니다. 컨테이너는 host directory와 Docker socket을 mount하지 않아 공개 저장소의 self-hosted runner 위험을 호스트와 격리합니다. BUSU는 전용 최소권한 계정으로 조회마다 새 PHP 세션을 만들고 이 세 화면만 순차 조회합니다. 자격증명은 `production` environment Secret에, 쿠키와 원문 HTML은 worker 메모리에만 두며 로그·artifact·DB에 보관하지 않습니다. Edge는 worker token과 4분 job lease를 다시 검증한 뒤 기존 parser와 upsert RPC만 실행합니다. CAPTCHA/MFA 또는 사람 확인 절차가 나타나면 우회하지 않습니다.

2026-08-15 운영 진단에서 로그인 GET의 HTTP `Set-Cookie`와 hidden `PHPSESSID`가 서로 다른 값임을 확인했습니다. parser `iping-4`는 전자를 Cookie, 후자를 로그인 POST form token으로 분리하고 로그인 뒤 회전된 Cookie를 사용합니다. 사용자 검색은 외부 아이핑 응답을 기다리지 않고 6시간 dedupe 큐에 등록하며, main 예약 worker가 10분마다 한 작업을 처리합니다. 인증·구조·접근 차단은 예약 실행에서 자동 반복하지 않고 기존 저장 기록을 유지합니다. backend 배포로 원인 수정이 반영된 경우에만 후속 workflow가 최근 결정적 실패 한 건을 한 번 복구해 새 Chrome 경로로 확인합니다.

2026-08-17 운영 재현에서 `source_auth_failed:login_verify`의 원인이 자격증명이 아니라 로그인 응답 구조임을 확인했습니다. 아이핑 로그인 POST는 HTTP 200으로 로그인 화면과 같은 비회원 문서를 먼저 돌려주고, 그 문서의 client redirect가 끝난 뒤에야 회원 홈이 됩니다. 이전 worker는 첫 문서를 그대로 판정해 로그인에 성공한 세션을 비회원으로 오인했고, 결정적 실패 회로가 6시간 동안 대기열 전체를 중단시켰습니다. `iping-5` worker는 회원 또는 사람 확인 화면이 확정될 때까지 최대 12초 동안 문서 교체를 견디며 다시 읽고, 사이트가 로그인 실패 alert를 띄운 경우에만 인증 실패로 종료합니다. 같은 점검에서 결과 표 끝에 붙는 빈 간격 행 때문에 parser가 정상 결과와 0건 결과를 모두 구조 변경으로 처리하던 문제도 확인해 내용 없는 행을 건너뛰도록 고쳤습니다. 검색어 자체를 아이핑이 거부해 `history.back`으로 돌려보내는 화면은 출처 전체 장애가 아니라 일시 실패로 분류해 회로를 열지 않습니다. 검색 쿼리와 응답은 계속 CP949/EUC-KR을 사용하며, 같은 검색어를 UTF-8로 보내면 오류 없이 0건이 돌아오므로 인코딩을 바꾸지 않습니다.
