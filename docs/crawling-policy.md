...
현재 live CLI는 `CRAWL_LIVE=true` 없이는 즉시 종료합니다. 애즈트리·대한탁구협회 디비전·마이티티·슈퍼스타탁구는 각각 대응하는 `CRAWLER_SOURCE_*_ENABLED=true`가 추가로 필요합니다. 일반 서버 호출은 같은 이름의 최근 6시간 성공 결과를 재사용할 수 있지만, 검색 화면은 사용자의 명시적 검색마다 `force=true`로 이 query cache를 우회합니다. 모든 호출에는 출처별 최소 요청 간격이 계속 적용됩니다. Edge 운영에서는 DB `sources.enabled=true`까지 필요합니다. 대한탁구협회 응답의 휴대폰·RT점수와 슈퍼스타의 레이팅 표는 저장하지 않습니다. 에어핑퐁과 오케이핑퐁은 parser가 준비됐더라도 이용약관의 사전 승낙 제한 때문에 운영 disabled를 유지합니다. 아이핑 선수 검색은 로그인이 필요하므로 인증 세션 자동화나 접근제어 우회를 구현하지 않습니다.

비활성 에어핑퐁·오케이핑퐁은 사용자 검색어를 각 사이트의 공개 검색 URL query에 넣은 외부 링크만 제공합니다. 이 경로에서는 BUSU 서버가 원문을 요청·파싱·저장하지 않습니다. 아이핑은 로그인 전용 검색이므로 메인 URL과 로그인 필요 상태만 표시합니다.
