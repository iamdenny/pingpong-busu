...
- 명시된 오픈·통합·지역·디비전 체계와 대회별 override를 지역 전환일 추정보다 우선한다.
- 대회 지역과 개최일이 모두 확인되면 버전 관리되는 지역별 전환일 전의 일반 부수는 지역부수, 당일부터는 통합부수로 분류한다. 둘 중 하나라도 없으면 일반 숫자 부수는 통합부수가 기본이다.
- 지역별 전환일 이전이거나 대회별 예외에 해당하는 지역부수 기록은 검색·상세 이력에 보존하되 현재 추정 부수·최근 관측 부수·최근 대회 요약에서는 제외한다. 제18회까지의 분당구청장기가 현재 예외이다.
