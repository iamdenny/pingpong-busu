...
- 홈의 Three.js 탁구 랠리는 한 줄 제목과 검색 영역 사이의 독립된 히어로 비주얼 스테이지로 사용하고 접근성 트리에서는 제외한다. 라켓면 각도는 임팩트 기준 드라이브 +20도(닫힘), 커트 −20도(열림)이며 백스윙에서는 55%까지 세워졌다가 임팩트에서 목표 각도에 도달한다. 두 선수의 리그는 Y축 90도로 이미 미러링되어 있으므로 각도 부호는 좌우 공통으로 둔다. 코스는 목표 레인의 좌우로 정하며, 공이 온 레인을 그대로 지키면 직선, 반대편으로 보내면 대각이 된다. 강제가 없을 때는 40% 확률로 직선을 고르고 나머지는 대각으로 보내 크로스가 자연스러운 다수로 남게 한다. 직전 두 타구가 같은 코스면 다음 코스를 반대쪽으로, 같은 타법이면 다음 타법을 반대로 강제한다. 직선을 두 번 이어 치면 같은 쪽으로 두 번 보낸 것이므로 세 번째 직선은 코스 규칙이 자동으로 막는다. 마우스 드래그와 한 손가락 터치는 가상 트랙볼로 라켓·공·네트를 포함한 랠리 월드만 3축 회전하고 바닥과 조명은 고정한다. 상판 법선이 카메라 반대편으로 넘어가는 회전은 막아 테이블 밑면을 노출하지 않는다. 포인터가 움직이는 중에 놓으면 마지막 회전축과 각속도를 이어받아 지수 감쇠(friction 2.4/s, 상한 7.5rad/s)로 관성 회전하고, 놓기 전 90ms 이상 멈춰 있었으면 관성을 주지 않는다. 관성 회전도 같은 밑면 노출 제한을 적용해 걸리면 즉시 멈춘다. 회전이 멎은 뒤 5초가 지나면 1.1초 easing으로 기본 쿼터니언과 기본 배율에 함께 복귀한다. 확대·축소는 프레이밍 거리를 나누는 배율(기본 1, 최소 0.8, 최대 2.2)로 두어 fit이 카메라를 밀어내기만 하는 성질을 유지한 채 확대가 fit을 넘어설 수 있게 한다. 휠 한 칸은 스크롤 거리의 지수로 계산해 확대·축소를 왕복하면 원래 배율로 돌아오며, 한 이벤트당 240px로 잘라 트랙패드 플링이 튀지 않게 한다. 배율이 한계에 걸린 휠은 `preventDefault`하지 않고 페이지 스크롤로 넘겨 히어로가 스크롤을 가두지 않는다. 다만 트랙패드 핀치는 `ctrlKey`가 붙은 휠로 들어오므로 항상 소비한다. 돌려주면 페이지가 스크롤되는 게 아니라 브라우저 전체가 확대되기 때문이며, 델타가 마우스 휠보다 훨씬 작아 별도 게인을 쓴다. 두 손가락은 하나의 제스처로 회전과 확대를 함께 수행한다. 두 손가락 중심점 이동이 아크볼 회전을, 두 손가락 간격이 배율을 담당한다. 핀치 배율은 제스처 시작 시점의 간격과 배율을 기준으로 하는 절대값이다. pointermove는 손가락별로 따로 도착해 한쪽만 갱신된 순간 간격이 튀는데, 증분 비율로 누적하면 그 흔들림이 배율 한계에서 상쇄되지 않고 영구히 밀린다. 손가락이 하나 떨어지면 남은 손가락 위치로 아크볼을 다시 심어 끊김 없이 한 손가락 회전으로 이어간다. 드래그하지 않을 때의 포인터 시차는 지연 보간하며 터치에서는 적용하지 않고, 모션 줄이기 환경에서는 자동 랠리를 멈추되 사용자가 직접 회전한 정지 장면은 즉시 렌더링하고 복귀는 즉시 적용한다. WebGL을 사용할 수 없거나 장면 로드가 실패해도 제목과 검색 기능은 그대로 제공한다.
