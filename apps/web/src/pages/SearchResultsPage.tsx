...
import { formatDivisionObservation } from '@busu/domain';
...
export function SearchResultsPage() {
  ...
  const shownCandidates = resultTab === 'awards' ? awardCandidates : entryCandidates;
  ...
  return <div className="page">
    ...
    {directSources.length > 0 && <aside className="direct-source-links" aria-label="원문 사이트 직접 검색"><strong>원문 직접 검색</strong><span>{directSources.map((source) => <a key={source.sourceCode} href={directSearchUrl(source.sourceCode, source.baseUrl, query)} target="_blank" rel="noreferrer">{source.displayName}{source.sourceCode === 'iping' ? ' (로그인)' : ''}</a>)}</span><small>링크만 제공하며 BUSU에 기록을 복제하지 않습니다.</small></aside>}
    ...
    {!result.isLoading && (result.data?.length ?? 0) > 0 && <div className="result-tabs" role="tablist" aria-label="검색 결과 구분"><button type="button" role="tab" aria-selected={resultTab === 'awards'} onClick={() => setResultTab('awards')}>입상 <strong>{awardCandidates.length}건</strong></button><button type="button" role="tab" aria-selected={resultTab === 'entries'} onClick={() => setResultTab('entries')}>출전 <strong>{entryCandidates.length}건</strong></button></div>}
    <section className="candidate-list" aria-label={`${resultTab === 'awards' ? '입상' : '출전'} 선수 검색 결과 목록`} role="tabpanel">
      {shownCandidates.map((player) => <article className="candidate-card" key={player.id}>
        ...
        <dl className="stats"><div><dt>최근 관측 부수</dt><dd>{formatDivisionObservation(player.recentObservedDivisionSystem, player.recentObservedDivision)}</dd></div><div><dt><Trophy size={15} /> 입상 기록</dt><dd>{player.resultCount}건</dd></div><div><dt><Waypoints size={15} /> 출처</dt><dd>{player.sourceCount}곳</dd></div></dl>
      ...
      {!result.isLoading && (result.data?.length ?? 0) > 0 && shownCandidates.length === 0 && <div className="empty-state"><h2>{resultTab === 'awards' ? '입상 기록이 없습니다.' : '출전 기록만 있는 후보가 없습니다.'}</h2><p>다른 탭에서 확인해 보세요.</p></div>}
    </section>
  </div>;
}
