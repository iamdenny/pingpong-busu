  ...
  render(
    <QueryClientProvider client={new QueryClient()}>
      <MemoryRouter initialEntries={[`/search?q=${encodeURIComponent(query)}`]}>
        <Routes><Route path="/search" element={<SearchResultsPage />} /></Routes>
      </MemoryRouter>
    </QueryClientProvider>,
  );
}
...
describe('SearchResultsPage', () => {
  it('shows dated award results, entry tabs, direct links, and whole-card detail links', async () => {
    ...
    expect(screen.getByRole('heading', { name: '“김탁구” 선수' })).toBeInTheDocument();
    expect(await screen.findAllByRole('heading', { name: '김탁구' })).toHaveLength(1);
    expect(screen.getByText('2건')).toBeInTheDocument();
    expect(screen.getByText(/우승 \(2026\. 7\. 20\.\).*3위 \(2026\. 4\. 6\.\).*외 1건/)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: '김탁구 서울 스핀탁구클럽 상세 기록 보기' })).toHaveAttribute('href', '/players/kim-seoul');
    expect(screen.queryByRole('link', { name: '상세 보기' })).not.toBeInTheDocument();

    const summary = screen.getByRole('region', { name: '현재 추정 부수' });
    expect(within(summary).getByRole('columnheader', { name: '오픈부수' })).toBeInTheDocument();
    expect(within(summary).getByRole('columnheader', { name: '통합부수' })).toBeInTheDocument();
    expect(within(summary).getByRole('cell', { name: '5부1건' })).toBeInTheDocument();
    expect(within(summary).getByRole('cell', { name: '6부1건' })).toBeInTheDocument();
    ...
    const list = screen.getByRole('tabpanel', { name: '출전 선수 검색 결과 목록' });
  ...
  it('filters same-name candidates when a region follows the player name', async () => {
    ...
    expect(screen.getByRole('heading', { name: '“김탁구” 선수 · 부산' })).toBeInTheDocument();
    const tabs = await screen.findByRole('tablist', { name: '검색 결과 구분' });
    expect(within(tabs).getByRole('tab', { name: '입상 0건' })).toBeDisabled();
    expect(within(tabs).getByRole('tab', { name: '출전 1건' })).toHaveAttribute('aria-selected', 'true');
    expect(screen.getByRole('link', { name: '김탁구 부산 블루라켓 상세 기록 보기' })).toBeInTheDocument();
    expect(screen.queryByText(/같은 이름의 선수가 여러 명/)).not.toBeInTheDocument();
  });
});
