...
export function PlayerDetailPage() {
  ...
  return <div className="page detail-page">
    ...
    <section className="division-summary"><article><span>최근 관측 부수</span><strong>{player.recentObservedDivisionSystem ? `${divisionSystemLabels[player.recentObservedDivisionSystem]} · ` : ''}{player.recentObservedDivision ?? '확인 필요'}</strong><small>해당 대회 기록 기준</small></article><article><span>지역부수 기록</span><strong>{player.sources.find((source) => source.recentObservedDivisionSystem === 'regional')?.recentObservedDivision ?? '확인 필요'}</strong><small>출처별 기준 상이</small></article><article><span>과거 입상 기록</span><strong>{player.resultCount}건</strong><small>4강 이상 · {player.dataKind === 'live' ? '수집된 공개 기록' : '가상 출처 포함'}</small></article></section>
