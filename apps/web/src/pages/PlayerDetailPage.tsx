...
import { divisionSystemLabels, isAwardRank, sortPlayerRecordsByLatest } from '@busu/domain';
...
export function PlayerDetailPage() {
  ...
  return <div className="page detail-page">
    <Link className="back-link" to={`/search?q=${encodeURIComponent(player.name)}`}><ArrowLeft size={18} /> 검색 결과로</Link>
    ...
    <section className="division-summary"><article><span>최근 관측 부수</span><strong>{player.recentObservedDivisionSystem ? `${divisionSystemLabels[player.recentObservedDivisionSystem]} · ` : ''}{player.recentObservedDivision ?? '확인 필요'}</strong><small>해당 대회 기록 기준</small></article><article><span>통합부수 기록</span><strong>{player.sources.find((source) => source.recentObservedDivisionSystem === 'integrated')?.recentObservedDivision ?? '확인 필요'}</strong><small>일반 시·군·구 대회 포함</small></article><article><span>과거 입상 기록</span><strong>{player.resultCount}건</strong><small>4강 이상 · {player.dataKind === 'live' ? '수집된 공개 기록' : '가상 출처 포함'}</small></article></section>
    ...
    {(tab === 'history' || tab === 'awards') && <section aria-labelledby="history-title"><h2 id="history-title" className="visually-hidden">대회 이력</h2><div className="record-table-wrap"><table><caption>{player.name} 선수의 대회 기록 · 최신순</caption><thead><tr><th>날짜</th><th>대회·종목</th><th>당시 소속</th><th>당시 부수</th><th>결과</th><th>출처</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td>{record.date ? <time dateTime={record.date}>{record.date}{record.dateBasis === 'published' && <small>게시일</small>}</time> : '날짜 미상'}</td><td><strong>{record.tournament}</strong><small>{record.event}</small></td><td>{record.club ?? '-'}</td><td>{record.divisionSystem ? <small>{divisionSystemLabels[record.divisionSystem]}</small> : null}{record.division ?? '-'}</td><td>{record.rank ?? '-'}</td><td><a href={record.sourceUrl} target="_blank" rel="noreferrer">{record.sourceName}<ExternalLink size={14} /></a></td></tr>)}</tbody></table></div><div className="record-cards">{records.map((record) => <article key={record.id}>{record.date ? <time dateTime={record.date}>{record.date}{record.dateBasis === 'published' && ' · 게시일'}</time> : <span className="record-date-unknown">날짜 미상</span>}<h3>{record.tournament}</h3><p>{record.event} · {record.club}</p><dl><div><dt>당시 부수</dt><dd>{record.divisionSystem ? `${divisionSystemLabels[record.divisionSystem]} · ` : ''}{record.division ?? '-'}</dd></div><div><dt>결과</dt><dd>{record.rank ?? '-'}</dd></div></dl><a href={record.sourceUrl} target="_blank" rel="noreferrer">{record.sourceName} 원문 <ExternalLink size={14} /></a></article>)}</div></section>}
