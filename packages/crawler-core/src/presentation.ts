import { compareNormalizedRecordsByLatest, isAwardRank, normalizedRecordDate, normalizeSearchText, stableHash, type NormalizedRecord, type PlayerDetail, type PlayerRecord, type SourceCode } from '@busu/domain';

function inferScale(tournamentName: string): PlayerRecord['scale'] {
  if (tournamentName.includes('전국')) return 'national';
  if (tournamentName.includes('도') || tournamentName.includes('광역시')) return 'province';
  if (tournamentName.includes('구') || tournamentName.includes('시')) return 'district';
  return 'unknown';
}

export function recordsToPlayerDetails(records: readonly NormalizedRecord[], sourceCode: SourceCode, sourceName: string): PlayerDetail[] {
  const groups = new Map<string, NormalizedRecord[]>();
  for (const record of records) {
    const identityKey = record.sourceIdentityKey ?? stableHash({ sourceCode, name: record.normalizedPlayerName, club: normalizeSearchText(record.clubText ?? '') });
    ...
    return {
      ...
      records: sorted.map((record) => {
        ...
        return {
          ...
          ...(date ? { date } : {}),
          ...(record.tournamentDate ? { dateBasis: 'tournament' as const } : record.sourcePublishedDate ? { dateBasis: 'published' as const } : {}),
          ...
          ...(record.divisionSystem ? { divisionSystem: record.divisionSystem } : {}),
