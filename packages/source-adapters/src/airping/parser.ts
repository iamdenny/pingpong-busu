...
function classContent(block: string, className: string): string {
  return (
    new RegExp(
      `<div\\b[^>]*class=(['"])[^'"]*\\b${className}\\b[^'"]*\\1[^>]*>([\\s\\S]*?)<\\/div>`,
      "iu",
    ).exec(block)?.[2] ?? ""
  );
}
...
function tournamentNameFromAnchor(anchorHtml: string): string {
  const afterMetadata = /<\/div>\s*([^<>]+?)\s*<\/div>\s*$/iu.exec(
    anchorHtml,
  )?.[1];
...
function parseEvents(
  block: string,
  playerName: string,
): AirpingParsedRow["events"] {
  const eventBlocks = block
    .split(/(?=<li\b[^>]*class=(['"])[^'"]*\bplayer_join_li\b)/giu)
    .filter((part) => /\bplayer_join_li\b/iu.test(part));
  ...
  return eventBlocks.flatMap((eventBlock) => {
    ...
    const eventName = stripHtml(eventHeading)
      .replace(/^\[(?:단식|복식|단체전?)\]\s*/u, "")
      .trim();
    if (!eventName) return [];
    const rankText = stripHtml(
      /<span\b[^>]*class=(['"])[^'"]*\bgame_result\b[^'"]*\1[^>]*>([\s\S]*?)<\/span>/iu.exec(
        eventBlock,
      )?.[2] ?? "",
    );
    const fullText = stripHtml(eventBlock);
    const divisionRaw = new RegExp(
      `${escapedName}\\s*[/／]\\s*([A-Za-z가-힣0-9]+)`,
      "iu",
    ).exec(fullText)?.[1];
...
export function parseAirpingSearchHtml(
  html: string,
  expectedName: string,
  observedAt: string,
): NormalizedRecord[] {
  if (
    !html.includes("playerSearchForm") ||
    !html.includes("_mc_content_list")
  ) {
    throw new SourceSchemaChangedError(
      "에어핑퐁 검색 페이지 식별자를 찾지 못했습니다.",
    );
  }
  if (html.includes("검색결과가 없습니다.") && !html.includes("player_join_li"))
    return [];

  const blocks = html
    .split(
      /(?=<li\b[^>]*class=(['"])[^'"]*\bplayer_cont_li\b[^'"]*\b_mc_div\b)/giu,
    )
    .filter(
      (block) =>
        /\bplayer_cont_li\b/iu.test(block) && !/\b_vc_fixed\b/iu.test(block),
    );
  const parsedRows: AirpingParsedRow[] = blocks.flatMap((block) => {
    ...
    const tournamentAnchor =
      /<div\b[^>]*class=(['"])[^'"]*\bplayer_inner4\b[^'"]*\1[^>]*>[\s\S]*?<a\b[^>]*href=(['"])(.*?)\2[^>]*>([\s\S]*?)<\/a\s*>/iu.exec(
        block,
      );
    const tournamentName = tournamentNameFromAnchor(
      tournamentAnchor?.[4] ?? "",
    );
    const tournamentDate = firstIsoDate(
      stripHtml(classContent(block, "player_inner5")),
    );
    const sourceUrl = tournamentAnchor?.[3]
      ? new URL(decodeHtml(tournamentAnchor[3]), AIRPING_SEARCH_URL).toString()
      : AIRPING_SEARCH_URL;
  ...
  return parsedRows.flatMap((row) => {
    ...
    return row.events.map((event) => {
      const region = inferKoreanRegion(
        row.regionEvidence,
        row.tournamentName,
        event.eventName,
      );
      return withRecordHashes({
        ...
        divisionSystem: inferRecordDivisionSystem({
          ...
          additionalEvidence: [row.regionEvidence, event.divisionValue],
        }),
