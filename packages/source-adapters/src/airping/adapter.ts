  ...
  async search(
    ...
    if (!this.enabled || !input.live)
      throw new SourceDisabledError(
        "에어핑퐁은 출처 운영자의 사전 승낙 전까지 비활성화됩니다.",
      );
    ...
    try {
      response = await fetchWithRetry(
        ...
        {
          timeoutMs: Math.max(context.timeoutMs, 16_000),
          maxAttempts: 2,
          retryDelayMs: 250,
        },
      );
    ...
    const records = [
      ...new Map(
        parseAirpingSearchHtml(html, input.name, fetchedAt).map((record) => [
