...
export class AstreeSourceAdapter implements SourceAdapter {
  ...
  readonly mode = 'http';
  readonly parserVersion = 'astree-5';
  ...
  async search(input: SourceSearchInput, context: SourceAdapterContext): Promise<SourceSearchResult> {
    if (!this.enabled || !input.live) throw new SourceDisabledError('애즈트리 live adapter가 비활성화되어 있습니다.');
    ...
    for (let page = 1; page <= maxPages; page += 1) {
      ...
      if (!response.ok) throw new SourceParseError(`애즈트리 HTTP ${response.status}`);
      const contentType = response.headers.get('content-type') ?? '';
      if (!contentType.toLocaleLowerCase().includes('text/html')) throw new SourceSchemaChangedError('HTML이 아닌 응답을 받았습니다.');
    ...
    return { sourceCode: 'astree', fetchedAt, sourceUrl: urls[0] ?? SEARCH_URL, records: uniqueRecords, warnings: [], rawContentHash: stableHash(pageContentHashes), parserVersion: this.parserVersion };
  }
}
