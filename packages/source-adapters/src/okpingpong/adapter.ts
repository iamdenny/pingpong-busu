...
export class OkPingpongSourceAdapter implements SourceAdapter {
  ...
  async search(input: SourceSearchInput, context: SourceAdapterContext): Promise<SourceSearchResult> {
    if (!this.enabled || !input.live) throw new SourceDisabledError('오케이핑퐁은 출처 운영자의 사전 승낙 전까지 비활성화됩니다.');
    ...
    try {
      ...
      throw new SourceParseError(error instanceof Error ? error.message : '오케이핑퐁 요청 실패');
    }
    if (response.status === 403) throw new SourceBlockedError();
    if (response.status === 429) throw new SourceRateLimitedError();
    if (!response.ok) throw new SourceParseError(`오케이핑퐁 HTTP ${response.status}`);
    ...
    const records = [...new Map(parseOkPingpongSearchHtml(html, input.name, fetchedAt).map((record) => [record.naturalKeyHash, record])).values()];
