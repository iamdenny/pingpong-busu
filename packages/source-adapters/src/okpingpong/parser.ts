...
export function parseOkPingpongSearchHtml(
  html: string,
  expectedName: string,
  observedAt: string,
): NormalizedRecord[] {
  if (
    ...
    !html.includes("result_tbl_total")
  ) {
    throw new SourceSchemaChangedError(
      "오케이핑퐁 검색 페이지 식별자를 찾지 못했습니다.",
    );
  }
  const table =
    /<table\b[^>]*class=(['"])[^'"]*\bresult_tbl_total\b[^'"]*\1[^>]*>([\s\S]*?)<\/table>/iu.exec(
      html,
    )?.[2];
  const tbody = table
    ? /<tbody\b[^>]*>([\s\S]*?)<\/tbody>/iu.exec(table)?.[1]
    : undefined;
  if (tbody === undefined)
    throw new SourceSchemaChangedError(
      "오케이핑퐁 결과 표 구조를 찾지 못했습니다.",
    );
  ...
  for (const row of rows) {
    const rowHtml = row[1] ?? "";
    if (stripHtml(rowHtml).includes("검색 결과가 없습니다")) continue;
    ...
    if (cells.length >= 8) {
      ...
      throw new SourceSchemaChangedError(
        "오케이핑퐁 결과 열 개수가 변경되었습니다.",
      );
    }
    const eventName = stripHtml(detailCells[1] ?? "")
    ...
    const partnerText = fullResult
      .replace(rankText, "")
      .replace(/:\s*$/u, "")
      .trim();
    parsedRows.push(
      okPingpongParsedRowSchema.parse({
        ...group,
        eventName,
        eventType: eventTypeFromText(stripHtml(detailCells[1] ?? "")),
        ...(normalizeObservedDivision(stripHtml(detailCells[0] ?? ""))
          ? {
              divisionValue: normalizeObservedDivision(
                stripHtml(detailCells[0] ?? ""),
              ),
            }
          : {}),
        ...(rankText ? { rankText } : {}),
    ...
    return [
      withRecordHashes({
        ...
        divisionSystem: inferRecordDivisionSystem({
          ...
          additionalEvidence: [row.category, row.divisionValue],
        }),
