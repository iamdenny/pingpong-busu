...
export class YonginTtSourceAdapter implements SourceAdapter {
  ...
  readonly mode = "http";
  readonly parserVersion = "yongintt-4";
  ...
  async search(
    ...
    if (!this.enabled || !input.live)
      throw new SourceDisabledError(
        "용인탁구협회 다음 카페 adapter가 비활성화되어 있습니다.",
      );
    if (!this.apiKey)
      throw new SourceDisabledError(
    ...
    for (let page = 1; page <= pageLimit; page += 1) {
      ...
      try {
        ...
        throw new SourceParseError(
          error instanceof Error ? error.message : "다음 카페 검색 요청 실패",
        );
      }
      if (response.status === 401 || response.status === 403)
        throw new SourceBlockedError("카카오 REST API 인증을 확인해 주세요.");
      if (response.status === 429)
        throw new SourceRateLimitedError(
          "카카오 무료 검색 쿼터 또는 요청 제한에 도달했습니다.",
        );
      if (!response.ok)
        throw new SourceParseError(`카카오 카페 검색 HTTP ${response.status}`);
      if (
        !(response.headers.get("content-type") ?? "")
          .toLocaleLowerCase()
          .includes("application/json")
      )
        throw new SourceSchemaChangedError("JSON이 아닌 응답을 받았습니다.");
    ...
    return {
      ...
      warnings: [],
      rawContentHash: stableHash(rawPages),
