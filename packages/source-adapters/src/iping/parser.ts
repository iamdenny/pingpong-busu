...
import { ipingParsedRowSchema, type IpingParsedRow, type IpingResultKind } from './schema';
...
function resultTable(html: string, resultKind: IpingResultKind): string {
  ...
  if (!table) throw new SourceSchemaChangedError(`아이핑 ${expectedHeading} 결과 표 구조를 찾지 못했습니다.`);
  return table;
}

function classContent(block: string, className: string): string {
  return new RegExp(`<span\\b[^>]*class=(['"])[^'"]*\\b${className}\\b[^'"]*\\1[^>]*>([\\s\\S]*?)<\\/span>`, 'iu').exec(block)?.[2] ?? '';
}
...
function parseResultRow(rowHtml: string, resultKind: IpingResultKind): IpingParsedRow | undefined {
  const cells = extractTableCells(rowHtml);
  if (cells.length === 0) return undefined;
  const rowText = stripHtml(rowHtml);
  if (rowText.includes('입상이력이 없습니다') || rowText.includes('출전이력이 없습니다')) return undefined;
  if (rowText.includes('선수명') && rowText.includes(resultKind === 'award' ? '입상이력' : '출전이력')) return undefined;
  if (cells.length !== 2) throw new SourceSchemaChangedError('아이핑 선수 결과 열 개수가 변경되었습니다.');

  const playerCell = cells[0] ?? '';
  const resultCell = cells[1] ?? '';
  const playerBoldValues = [...playerCell.matchAll(/<b\b[^>]*>([\s\S]*?)<\/b>/giu)].map((match) => stripHtml(match[1] ?? ''));
  const playerName = playerBoldValues[0] ?? '';
  const divisionValue = normalizeObservedDivision(playerBoldValues[1]?.replace(/[()]/gu, ''));
  const clubText = stripHtml(classContent(playerCell, 'text13g'));

  const detailAnchors = [...resultCell.matchAll(/<a\b[^>]*href=(['"])([^'"]*pg=CVR[^'"]*)\1[^>]*>([\s\S]*?)<\/a>/giu)];
  const tournamentAnchor = detailAnchors[0];
  const eventAnchor = detailAnchors[1];
  if (!tournamentAnchor?.[2] || !eventAnchor?.[3]) throw new SourceSchemaChangedError('아이핑 대회 결과 링크 구조가 변경되었습니다.');
  const tournamentName = stripHtml(tournamentAnchor[3] ?? '');
  const eventWithRank = stripHtml(eventAnchor[3]);
  const rankValues = [...eventAnchor[3].matchAll(/<b\b[^>]*>([\s\S]*?)<\/b>/giu)].map((match) => stripHtml(match[1] ?? '')).filter(Boolean);
  const rankText = resultKind === 'award' ? rankValues.at(-1) : undefined;
  if (resultKind === 'award' && !rankText) throw new SourceSchemaChangedError('아이핑 입상 순위 구조가 변경되었습니다.');
  const eventName = rankText
    ? eventWithRank.replace(new RegExp(`${escapeRegExp(rankText)}\\s*$`, 'u'), '').trim()
    : eventWithRank;
  const tournamentDate = firstIpingDate(rowText);
  if (!playerName || !tournamentName || !eventName || !tournamentDate) throw new SourceSchemaChangedError('아이핑 선수 결과 필수 항목을 찾지 못했습니다.');
  const tournamentScale = /시군구대회/u.test(rowText) ? '시군구대회' : /전국오픈/u.test(rowText) ? '전국오픈' : '대회 체계 확인 필요';
  const sourceUrl = new URL(decodeHtml(tournamentAnchor[2]), IPING_BASE_URL).toString();
...
export function parseIpingSearchHtml(
  ...
  if (!html.includes('아이핑검색') || !html.includes(resultKind === 'award' ? '입상이력' : '출전이력')) {
    throw new SourceSchemaChangedError('아이핑 검색 페이지 식별자를 찾지 못했습니다.');
  }
  ...
  return parsedRows.flatMap((row) => {
    ...
    const sourceIdentityKey = stableHash({
      ...
      ...(row.clubText ? { club: normalizeSearchText(row.clubText) } : { tournament: normalizeSearchText(row.tournamentName) }),
    });
    const region = inferKoreanRegion(row.tournamentName, row.eventName, row.clubText);
