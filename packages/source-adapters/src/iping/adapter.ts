...
import { IPING_BASE_URL, IPING_SEARCH_URL, parseIpingSearchHtml } from './parser';
...
function assertHtmlResponse(response: Response, label: string): void {
  ...
  if (!response.ok) throw new SourceParseError(`아이핑 ${label} HTTP ${response.status}`);
  if (!(response.headers.get('content-type') ?? '').toLocaleLowerCase().includes('text/html')) {
    throw new SourceSchemaChangedError('아이핑에서 HTML이 아닌 응답을 받았습니다.');
  }
}
...
async function request(
  ...
  try {
    return await fetch(url, {
      ...init,
      signal: requestSignal(input, context),
      headers: {
        accept: 'text/html',
    ...
    if (input.signal?.aborted || (error instanceof DOMException && error.name === 'TimeoutError')) throw new SourceTimeoutError();
    throw new SourceParseError(error instanceof Error ? error.message : '아이핑 요청 실패');
  }
}
...
async function createAuthenticatedSession(
  ...
  const loginPage = await request(loginUrl, input, context, { redirect: 'manual' });
  assertHtmlResponse(loginPage, '로그인 화면');
  const initialCookie = responseCookie(loginPage);
  if (!initialCookie) throw new SourceSchemaChangedError('아이핑 로그인 세션 쿠키를 찾지 못했습니다.');
  const body = encodeIpingForm({ path: '', pg: 'login', Mid: credentials.username, Pwd: credentials.password });
  const loginResponse = await request(loginUrl, input, context, {
    method: 'POST',
    redirect: 'manual',
    body,
    headers: {
  ...
  if (loginResponse.status >= 400) throw new SourceParseError(`아이핑 로그인 HTTP ${loginResponse.status}`);
  ...
  const verificationResponse = await request(destination, input, context, { headers: { cookie: sessionCookie, referer: loginUrl }, redirect: 'follow' });
  assertHtmlResponse(verificationResponse, '로그인 확인');
  const verificationHtml = await responseHtml(verificationResponse);
  if (!verificationHtml.includes('mb_logout.php')) throw new SourceBlockedError('아이핑 계정 인증에 실패했습니다.');
...
async function fetchSearchHtml(
  ...
  const response = await request(url, input, context, { headers: { cookie, referer: IPING_SEARCH_URL }, redirect: 'follow' });
  assertHtmlResponse(response, '선수 검색');
  const html = await responseHtml(response);
  if (html.includes('name="Mid"') && html.includes('name="Pwd"')) throw new SourceBlockedError('아이핑 인증 세션이 만료되었습니다.');
  return html;
}
...
export class IpingSourceAdapter implements SourceAdapter {
  ...
  readonly mode = 'http';
  readonly parserVersion = 'iping-1';

  constructor(readonly enabled = false, private readonly credentials?: IpingCredentials) {}
  ...
  async search(input: SourceSearchInput, context: SourceAdapterContext): Promise<SourceSearchResult> {
    if (!this.enabled || !input.live) throw new SourceDisabledError('아이핑 live adapter가 비활성화되어 있습니다.');
    if (!this.credentials?.username || !this.credentials.password) throw new SourceDisabledError('아이핑 전용 계정 Secret이 설정되지 않았습니다.');
    const cookie = await createAuthenticatedSession(this.credentials, input, context);
    const [entriesHtml, nationwideAwardsHtml, districtAwardsHtml] = await Promise.all([
    ...
    const records = [
      ...
      ...parseIpingSearchHtml(nationwideAwardsHtml, input.name, fetchedAt, 'award'),
      ...parseIpingSearchHtml(districtAwardsHtml, input.name, fetchedAt, 'award'),
    ];
    ...
    return {
      ...
      rawContentHash: stableHash(`${entriesHtml}\n${nationwideAwardsHtml}\n${districtAwardsHtml}`),
