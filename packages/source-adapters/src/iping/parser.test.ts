...
const fixtureDirectory = resolve(import.meta.dirname, '../../../../fixtures/sources/iping');
const awardsFixture = readFileSync(resolve(fixtureDirectory, 'awards.html'), 'utf8');
const entriesFixture = readFileSync(resolve(fixtureDirectory, 'entries.html'), 'utf8');

describe('parseIpingSearchHtml', () => {
  it('normalizes nationwide and district awards from the authenticated result table', () => {
    const records = parseIpingSearchHtml(awardsFixture, '홍라켓', '2026-08-12T00:00:00.000Z', 'award');
  ...
  it('normalizes participation rows without inventing a rank', () => {
    const records = parseIpingSearchHtml(entriesFixture, '홍라켓', '2026-08-12T00:00:00.000Z', 'entry');
  ...
  it('rejects a login page instead of treating it as an empty result', () => {
    expect(() => parseIpingSearchHtml('<form><input name="Mid"><input name="Pwd"></form>', '홍라켓', '2026-08-12T00:00:00.000Z', 'award'))
