...
function assertResponse(response: Response, label: string): void {
  ...
  if (!response.ok)
    throw new SourceParseError(`마이티티 ${label} HTTP ${response.status}`);
  if (
  ...
  async search(
    ...
    if (!this.enabled || !input.live)
      throw new SourceDisabledError(
        "마이티티 live adapter가 비활성화되어 있습니다.",
      );
    ...
    try {
      pageResponse = await fetch(MYTT_SEARCH_URL, {
        signal,
        headers,
        redirect: "follow",
      });
      ...
      throw new SourceParseError(
        error instanceof Error ? error.message : "마이티티 검색 form 요청 실패",
      );
    }
    ...
    try {
      ...
      if (signal.aborted) throw new SourceTimeoutError();
      throw new SourceParseError(
        error instanceof Error ? error.message : "마이티티 검색 요청 실패",
      );
    }
    ...
    const records = [
      ...new Map(
        parseMyttSearchHtml(html, input.name, fetchedAt).map((record) => [
