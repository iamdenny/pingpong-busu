...
const fixture = (name: string): string =>
  readFileSync(
    resolve(import.meta.dirname, "../../../../fixtures/sources/mytt", name),
    "utf8",
  );

describe("마이티티 parser", () => {
  it("extracts JSF form fields without persisting a session URL", () => {
    expect(parseMyttSearchForm(fixture("search-form.html"))).toEqual({
      viewState: "synthetic-view-state",
      submitButton: "mainForm:search",
    });
  });

  it("parses synthetic participant and award rows", () => {
    const records = parseMyttSearchHtml(
    ...
    expect(records[0]?.sourceUrl).toBe(
      "https://mytt.kr/main/player_list.xhtml",
    );
  });
