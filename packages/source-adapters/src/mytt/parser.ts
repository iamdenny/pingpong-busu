...
import {
  myttParsedRowSchema,
  myttSearchFormSchema,
  type MyttParsedRow,
} from "./schema";

export const MYTT_SEARCH_URL = "https://mytt.kr/main/player_list.xhtml";

export function parseMyttSearchForm(html: string): {
  viewState: string;
  submitButton: string;
} {
  if (
    !html.includes("mainForm:playerName") ||
    !html.includes("javax.faces.ViewState")
  ) {
    throw new SourceSchemaChangedError(
      "마이티티 공개 검색 form 식별자를 찾지 못했습니다.",
    );
  }
  const viewStateMatches = [
    ...html.matchAll(
      /name=["']javax\.faces\.ViewState["'][^>]*value=["']([^"']+)["']/giu,
    ),
  ];
  const viewState = viewStateMatches.at(-1)?.[1];
  const submitButton =
    /<button\b[^>]*id=["'](mainForm:[^"']+)["'][^>]*>[\s\S]*?<span\b[^>]*>\s*검색\s*<\/span>/iu.exec(
      html,
    )?.[1];
  return myttSearchFormSchema.parse({ viewState, submitButton });
}

function playerFromCell(
  playersText: string,
  expectedName: string,
): Pick<MyttParsedRow, "playerName" | "divisionValue"> | undefined {
...
export function parseMyttSearchHtml(
  html: string,
  expectedName: string,
  observedAt: string,
): NormalizedRecord[] {
  if (
    !html.includes("mainForm:playerTable") ||
    !html.includes("mainForm:playerTable_data")
  ) {
    throw new SourceSchemaChangedError(
      "마이티티 결과 표 식별자를 찾지 못했습니다.",
    );
  }
  const tbody =
    /<tbody\b[^>]*id=["']mainForm:playerTable_data["'][^>]*>([\s\S]*?)<\/tbody>/iu.exec(
      html,
    )?.[1];
  if (tbody === undefined)
    throw new SourceSchemaChangedError(
      "마이티티 결과 표 구조를 찾지 못했습니다.",
    );
  if (
    tbody.includes("ui-datatable-empty-message") &&
    tbody.includes("No records found")
  )
    return [];
  ...
  const parsedRows: MyttParsedRow[] = rows.flatMap((row) => {
    ...
    return [
      myttParsedRowSchema.parse({
        ...
        ...(clubText ? { clubText } : {}),
        ...player,
        ...(eventType === "team" || eventType === "doubles"
          ? { partnerText: playersText }
          : {}),
      }),
    ];
  });
  ...
  return parsedRows.map((row) => {
    ...
    return withRecordHashes({
      ...
      divisionSystem: inferRecordDivisionSystem({
        ...
        additionalEvidence: [row.scale, row.divisionValue],
      }),
      ...(row.divisionValue ? { divisionValue: row.divisionValue } : {}),
      ...(row.rankText ? { rankText: row.rankText } : {}),
      ...(row.partnerText ? { partnerText: row.partnerText } : {}),
      sourceUrl: MYTT_SEARCH_URL,
      observedAt,
    });
  });
}
