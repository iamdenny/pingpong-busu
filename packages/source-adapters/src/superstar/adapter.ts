...
export class SuperstarSourceAdapter implements SourceAdapter {
  ...
  readonly mode = "http";
  readonly parserVersion = "superstar-2";
  ...
  async search(
    ...
    if (!this.enabled || !input.live)
      throw new SourceDisabledError(
        "슈퍼스타탁구 live adapter가 비활성화되어 있습니다.",
      );
    ...
    try {
      ...
      throw new SourceParseError(
        error instanceof Error ? error.message : "슈퍼스타탁구 검색 요청 실패",
      );
    }
    if (response.status === 403) throw new SourceBlockedError();
    if (response.status === 429) throw new SourceRateLimitedError();
    if (!response.ok)
      throw new SourceParseError(`슈퍼스타탁구 HTTP ${response.status}`);
    if (
    ...
    const records = [
      ...new Map(
        parseSuperstarSearchHtml(html, input.name, fetchedAt).map((record) => [
