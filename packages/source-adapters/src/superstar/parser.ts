...
export const SUPERSTAR_SEARCH_URL =
  "https://www.superstar.kr/open/Do.jsp?urlSeq=302";

function splitTournamentAndEvent(value: string): {
  tournamentName: string;
  eventName: string;
} {
  const marker = /\s(?=(?:개인|남자|여자|혼성)?(?:단식|복식|단체전))/u.exec(
    value,
  );
  if (!marker || marker.index < 4)
    return { tournamentName: value, eventName: value };
  return {
...
function resultTable(html: string): string {
  ...
  if (!table)
    throw new SourceSchemaChangedError(
      "슈퍼스타 개인별 결과 표 구조를 찾지 못했습니다.",
    );
  return table;
}

export function parseSuperstarSearchHtml(
  html: string,
  expectedName: string,
  observedAt: string,
): NormalizedRecord[] {
  if (!html.includes("개인별 결과") || !html.includes("탁구대회 성적")) {
    throw new SourceSchemaChangedError(
      "슈퍼스타 개인별 결과 페이지 식별자를 찾지 못했습니다.",
    );
  }
  const echoedName =
    /<input\b[^>]*name=["']userNm["'][^>]*value=["']([^"']*)["']/iu.exec(
      html,
    )?.[1];
  if (
    echoedName === undefined ||
    normalizePlayerName(decodeHtml(echoedName)) !==
      normalizePlayerName(expectedName)
  ) {
    throw new SourceSchemaChangedError(
      "슈퍼스타 검색 응답의 선수 이름을 확인하지 못했습니다.",
    );
  }
  const rows = [
    ...resultTable(html).matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/giu),
  ];
  ...
  return parsedRows.map((row) => {
    ...
    return withRecordHashes({
      ...
      divisionSystem: inferRecordDivisionSystem({
        ...
        additionalEvidence: [row.divisionValue],
      }),
