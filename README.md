...
- 홈 제목 아래의 독립된 히어로 스테이지에서 드라이브·커트, 포핸드·백핸드와 공의 회전을 반복하는 Three.js 탁구 랠리. 마우스 드래그와 한 손가락 터치로 고정 바닥을 제외한 랠리 월드를 3축 회전하되 테이블 밑면은 노출하지 않고, 빠르게 튕기면 관성으로 이어 돌다 서서히 멈춘 뒤 5초가 지나면 기본 시점으로 부드럽게 복귀한다. 마우스 휠과 두 손가락 핀치로 0.8배에서 2.2배까지 확대·축소하고, 배율 한계에서는 휠을 페이지 스크롤로 넘긴다. WebGL 미지원 환경에서는 검색 콘텐츠만 유지하고 모션 줄이기 환경에서는 정지 장면으로 표시
