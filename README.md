...
- 홈 제목 아래의 독립된 히어로 스테이지에서 드라이브·커트, 포핸드·백핸드와 공의 회전을 반복하는 Three.js 탁구 랠리. 마우스 드래그와 한 손가락 터치로 고정 바닥을 제외한 랠리 월드를 턴테이블처럼 회전하되 가로는 제한 없이 돌고 세로는 밑면이 보이지 않는 범위로 제한하며, 빠르게 튕기면 관성으로 이어 돌다 서서히 멈춘 뒤 5초가 지나면 기본 시점으로 부드럽게 복귀한다. 마우스 휠, 트랙패드 핀치, 두 손가락 터치로 0.8배에서 2.2배까지 확대·축소하며 두 손가락은 회전과 확대를 동시에 수행하고, 배율 한계에서는 휠을 페이지 스크롤로 넘긴다. 공이 라켓과 테이블에 맞는 타구음을 오디오 파일 없이 합성해 낼 수 있고 기본은 켜짐이며 장면 위 스피커 아이콘 버튼으로 끄고 켜고, 선택은 브라우저에 저장한다. 브라우저 정책상 실제 재생은 첫 사용자 동작부터 시작한다. WebGL 미지원 환경에서는 검색 콘텐츠만 유지하고 모션 줄이기 환경에서는 정지 장면으로 표시
...
- 모바일 검색창은 16px 이상 입력 글자와 Rally Blue 포커스 테두리를 유지하고 바깥 크기가 커지는 효과를 쓰지 않으며, 브라우저의 사용자 확대 기능을 제한하지 않음
...
Supabase repository는 공개 검색/상세 view와 refresh Edge Function을 사용합니다. [두 번째 migration](./supabase/migrations/202608120002_astree_refresh.sql)이 identity, revision upsert RPC와 상세 view를 추가합니다. Edge 환경에 `CRAWL_LIVE=true`, 활성 출처별 환경 변수를 설정하고 DB의 `sources.enabled`도 true로 둬야 실제 요청이 활성화됩니다. 배포 workflow는 루트 package 버전으로 `CRAWLER_USER_AGENT=BUSU/{version}`을 설정합니다. 마이티티는 단기 JSF 세션을 검색 요청에만 쓰고 저장하지 않으며, 슈퍼스타탁구는 공개 개인별 결과 GET 검색만 사용합니다. 뉴티티플레이는 공개 탁구인검색 결과를 최대 2페이지만 처리하는 production opt-in 출처입니다. 용인탁구협회 카페는 카카오 공식 카페 검색 API를 검색당 1회 호출하고 용인 카페 URL만 남깁니다. 아이핑은 브라우저 요청에서 외부 조회를 실행하지 않고 선수 이름 형태만 허용하는 private queue에 등록합니다. 최대 12개의 활성 작업과 분당 신규 4건 admission budget을 두며 main 예약 worker가 5분마다 한 작업을 처리합니다. worker는 서버 전용 계정으로 매 조회마다 새 PHP 세션을 만들고 CP949 검색 결과를 출전·전국 입상·지역 입상 순서로 처리하되 세션 식별자와 자격증명은 저장하지 않습니다. 일시 오류만 15~60분 backoff로 최대 3회 시도하고 인증·구조·접근 차단은 backlog를 종료한 뒤 6시간 회로를 엽니다. backend 배포가 성공하면 최근 결정적 실패 한 건을 한 번 복구해 새 worker를 검증하며, 복구 대상이 없으면 외부 조회 없이 끝납니다. 일반 동기 출처에는 `출처 + 정규화 검색어`별 호출 제한을 적용하며, 에어핑퐁과 뉴티티플레이는 출처 전체 분당 6회 예산을 적용합니다. 에어핑퐁 Edge 요청은 10초 안에 응답하지 않으면 한 번의 서버 시도를 종료하고, 화면이 5초 이상 기다린 뒤 최대 2회 다시 요청합니다. workspace live CLI adapter는 별도 진단 도구이므로 에어핑퐁 GET에 16초 제한과 일시 오류 1회 재시도를 유지합니다.
...
동명이인이 많으면 `이름 지역` 형식으로 검색합니다. 예를 들어 `김미진 용인`은 외부 출처에는 `김미진`만 조회하고, BUSU 저장 후보를 지역에 `용인`이 포함된 결과로 좁힙니다. 출처에 표시된 소속은 상세 원문 근거로 보존하지만 검토 없이 검색 카드의 대표 소속이나 canonical club으로 자동 승격하지 않습니다. 입상 후보 카드에는 최근 입상부터 등수와 그 다음 줄의 대회일을 최대 두 건 표시하고 나머지는 `외 N건`으로 줄입니다. 반복되는 `상세 보기` 버튼 대신 카드 전체가 해당 후보 상세 링크입니다. 검색 결과가 한 건 이상이면 `별칭으로 기록 묶기`를 열어 각 후보의 최근 출전 대회명과 원문 종목명을 확인하고 추천 목록에서 무작위로 제시된 첫 별칭을 수정하거나 원하는 별칭을 직접 입력해 기록을 배정할 수 있습니다. 단일 후보는 유일한 별칭 그룹에 기본 배정하고, 복수 후보는 사용자가 소속과 활동 지역을 확인해 직접 배정합니다. 검색 결과가 많아도 실제로는 한 사람일 수 있으므로 `전체 N건 한 사람으로` 버튼으로 모든 후보를 한 별칭에 한 번에 배정하고 `전체 해제`로 되돌릴 수 있으며, 이때도 별칭 입력과 확인 동의를 거쳐 제출합니다. 저장된 별칭이 있으면 다음에 창을 열 때 사람 그룹과 기록 선택을 복원합니다. 전용 RPC가 아직 배포되지 않았거나 일시 실패하면 기존 공개 결과 조회로 최근 기록을 대체하며 화면에서 다시 불러올 수도 있습니다. 후보 수에는 고정 상한이 없고 확실하지 않은 기록은 미분류로 남깁니다. 편집은 즉시 반영되며 검색 화면의 `참여 편집 이력`에서 후보별 별칭을 확인하고 전체 원복할 수 있습니다. 별칭은 본인 인증이나 실제 실력·부수·공식 등급이 아니라 공개 기록을 묶고 구분하는 이름입니다.
...
아이핑 queue admission은 원본 IP를 저장하지 않고 service-role HMAC으로 만든 요청 원점 해시별 10분 4건 제한을 전역 제한과 함께 적용합니다. 이 해시는 하루 안에 삭제합니다.
